...
class DesConnectorSpec extends UnitSpec with MockitoSugar{
  class DefaultDesConnectorMock(
   ...
   ) extends DefaultDesConnector(httpClient, runModeConfig, environment) {
  ...
  "URLs and URIs" should {
    val desConnectorMock = new DefaultDesConnectorMock(mock[HttpClient], mock[Configuration], mock[Environment])
